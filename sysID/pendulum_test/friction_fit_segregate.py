...
def train_and_analyze(data, dataset_name, omega_mask=None, capture_output=False):
    ...
    for order in range(1, 4):  # 1, 2, 3 (User requested max order 3)
        # Full Poly
    ...
    def analyze(model_struct, name, min_contrib=1e-9):
        ...
        for s in stats:
            if s["AvgAbsContrib"] > min_contrib:
                ...
                count += 1
                if count >= 12: # Slightly higher limit
                    pass 
        log("-" * 50)

    analyze(models_out['best_full'], f"Best Full", min_contrib=1e-9)
...
def print_three_columns(col1, col2, col3, width=55):
    max_len = max(len(col1), len(col2), len(col3))
    
    # Headers
    print(f"\n{'Positive (+)':<{width}} | {'Negative (-)':<{width}} | {'Combined (Full)':<{width}}")
    print("-" * (width * 3 + 6))
    ...
    for i in range(max_len):
        ...
        # Truncate
        c1 = (c1[:width-3] + "..") if len(c1) > width else c1
        c2 = (c2[:width-3] + "..") if len(c2) > width else c2
        c3 = (c3[:width-3] + "..") if len(c3) > width else c3
        
        print(f"{c1:<{width}} | {c2:<{width}} | {c3:<{width}}")
...
def main():
    ...
    # Combined masks (all True)
    mask_u_all = np.ones(len(data_unlubed['friction']), dtype=bool)
    mask_l_all = np.ones(len(data_lubed['friction']), dtype=bool)
    
    print("\n" + "="*160)
    print("ANALYSIS: POSITIVE vs NEGATIVE vs COMBINED")
    print("="*160)

    # 1. UNLUBED Comparison
    print(f"\n>>> UNLUBED DATA SET")
    ...
    models_u_all, logs_u_all = train_and_analyze(data_unlubed, "Unlubed (ALL)", mask_u_all, capture_output=True)
    
    print_three_columns(logs_u_pos, logs_u_neg, logs_u_all, width=58)
    ...
    models_l_all, logs_l_all = train_and_analyze(data_lubed, "Lubed (ALL)", mask_l_all, capture_output=True)
    
    print_three_columns(logs_l_pos, logs_l_neg, logs_l_all, width=58)

    
    # Grid Plotting (Keeping Segregated 2x2 as requested)
    ...
    print("\nGenerating 2x2 Grid Plots (Segregated Only)...")
