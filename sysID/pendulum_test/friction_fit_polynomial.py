...
def main():
    ...
    # Model Strategy 1: Full Polynomial (with cross terms)
    # Model Strategy 2: Additive (no cross terms)
    ...
    for order in range(1, 5): # 1, 2, 3, 4
        ...
        # Features: [S, S^2...S^n, C, C^2...C^n, W, W^2...W^n]
        # Equivalent to hstack of single-variable expansions
        
        # We handle Order 1 specially: it's identical to Strategy 1 (just S, C, W)
        if order == 1:
            # Identical to full poly order 1
            results.append({
            ...
            for power in range(1, order + 1):
                X_add_list.append(X_base**power) # Adds columns [S^p, C^p, W^p]
