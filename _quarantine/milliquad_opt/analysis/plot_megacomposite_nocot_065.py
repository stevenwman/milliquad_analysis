"""Megacomposite (no COT) with step pitch gated at 65%.

Copy of plot_megacomposite_nocot.py — identical except step terrain sim pitch
uses 65% spatial gate (83.5mm) instead of 90% (96.4mm). Flat/rough unchanged.

Usage:
    uv run python -m analysis.plot_megacomposite_nocot_065 \
...
# Step terrain: recompute pitch_rms at 65% spatial gate from NPZ trajectories
_STEP_START_X = 0.05
_STEP_END_X = 0.1015
_CUTOFF_065 = _STEP_START_X + 0.65 * (_STEP_END_X - _STEP_START_X)


def _recompute_step_065(rows: list[dict], run_dir: pathlib.Path):
    """Override vx and pitch_rms in row dicts using 65% spatial gate from NPZ data."""
    import csv as _csv
    npz_files = sorted(run_dir.glob("*_validation_trajectories.npz"))
    if not npz_files:
        print(f"WARNING: no NPZ in {run_dir}, keeping 90% values")
        return
    d = np.load(str(npz_files[-1]), allow_pickle=True)
    ...
    for i, row in enumerate(rows):
        ...
        row["pitch_rms"] = float(np.std(p_gate - p_gate[0]))

        # Velocity: displacement / time over 65% gate
...
def main():
    ...
    out = args.output or "plots/megacomposite_nocot_065.png"
