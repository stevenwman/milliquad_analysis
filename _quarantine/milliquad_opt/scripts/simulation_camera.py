...
def run_simulation(
    ...
    Args:
        ...
        spawn_offset: (dx, dy, dz) position offset (rough terrain).
        cam_azimuth: Camera azimuth angle in degrees (None = MuJoCo default).
        cam_elevation: Camera elevation angle in degrees (None = MuJoCo default).
        cam_distance: Camera distance from target (None = config default).
        cam_lookat: Camera lookat offset (x, y, z). In tracking mode, offsets from
            tracked body. In fixed mode, sets absolute world lookat point.
        cam_tracking: If True, camera tracks body 1. If False, use fixed camera.
        slow_mo: Slow-motion multiplier (2.0 = half speed). Capture fps is
            auto-computed as 30*slow_mo, capped at sim rate (1/SIM_TIMESTEP).
    ...
    if record_path:
        ...
        cam.distance = cam_distance if cam_distance is not None else CAMERA_DISTANCE_RECORD
    ...
    try:
        ...
        if visualize and not record_path:
            ...
            with mujoco.viewer.launch_passive(model, data, key_callback=key_callback) as viewer:
                ...
                viewer.cam.distance = cam_distance if cam_distance is not None else CAMERA_DISTANCE_VIEWER
                if cam_azimuth is not None:
            ...
            _capture_fps = min(30.0 * slow_mo, 1.0 / SIM_TIMESTEP)
            frame_time_step = 1.0 / _capture_fps
            next_frame_time = 0.0
            step_times = None
            if benchmark:
    ...
    if record_path:
        _cap_fps = min(30.0 * slow_mo, 1.0 / SIM_TIMESTEP)
        playback_fps = _cap_fps / slow_mo
        _write_video(record_path, frames, playback_fps)
