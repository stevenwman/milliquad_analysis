"""Plot COT lower bound from experimental kinematics.

2-row figure (flat, step) matching the style of plot_cot_upper_bound.py:
...
from cot_lower_bound import compute_trial_cot_lower
...
def _collect_cot(conditions, csv_dir, terrain):
    """Collect per-trial COT lower bound values into {morph: {freq: [cot_values]}}."""
    data = {}
...
def main():
    ...
    # Labels
    ax_flat.set_ylabel("COT lower bound", fontsize=9)
    ax_step.set_ylabel("COT lower bound", fontsize=9)
    ...
    fig.suptitle("Exp COT Lower Bound (KE only)", fontsize=10, fontweight="bold")
    ...
    out = Path(__file__).resolve().parent / "cot_lower_bound.png"
