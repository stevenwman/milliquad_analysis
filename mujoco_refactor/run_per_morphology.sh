#!/usr/bin/env bash
# Run independent optimizations split by morphology and/or frequency.
# Usage: bash run_per_morphology.sh [n_calls] [--loop-morphs | --loop-freqs]
set -euo pipefail
cd "$(dirname "$0")"

N=${1:-600}
MODE=${2:-all}  # all, --loop-morphs, --loop-freqs
