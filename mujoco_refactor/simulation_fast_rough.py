"""
Core MuJoCo simulation engine for rough terrain evaluation.

Based on simulation_fast_new.py (16-param extended version), with two changes:
- spawn_offset support: places robot at arbitrary (x, y, z) offset
- No yaw jitter (not needed for rough terrain Y-jitter trials)

Usage:
    from simulation_fast_rough import run_simulation
    trajectory = run_simulation(params, sim_duration=2.0, spawn_offset=(0.05, 0.003, 0.01))
"""
...
def _initialize_pose(
    ...
    """Initialize the robot pose with optional spawn offset.

    Args:
        spawn_offset: (dx, dy, dz) offset applied after default pose setup.
    """
...
def run_simulation(
    ...
    Args:
        params: Simulation parameters dict. Expected keys:
            ...
            noslip_iterations (optional), noslip_tolerance (optional), margin (optional)
        ...
        spawn_offset: (dx, dy, dz) offset for robot initial position.
    ...
    # NEW: Apply additional solver parameters (optional, backward compatible)
    ...
    if 'margin' in params:
        model.opt.o_margin = float(params['margin'])  # Attribute is o_margin, not margin
    ...
    _initialize_pose(data, spawn_offset=spawn_offset)
    ...
    if progress and sim_duration > 0:
        ...
        print(f"[sim pid={os.getpid()} scene={scene_name}] start")
    try:
        mujoco.mj_step(model, data)
            ...
            while data.time < sim_duration:
                ...
                if progress_thresholds is not None:
                    ...
                    while (
                        ...
                        print(
                            f"[sim pid={os.getpid()} scene={scene_name}] {pct}% "
