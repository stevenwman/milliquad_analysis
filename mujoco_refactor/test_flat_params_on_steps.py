...
# Load best params from ongoing flat optimization
BEST_RUN = "results/20260223T144835_corrected_flip_20hz_cold"
...
def compute_cost_components(trajectory, target_vel, step_start_x=STEP_START_X):
    ...
    vel_cost = abs(vel - target_vel) / target_vel if target_vel > 0 else abs(vel)
...
for scene, freq, robot_dir in TEST_CONDITIONS:
    ...
    best_trial = min(trial_results, key=lambda x: x["cost"])
    ...
    print(f"{scene_id:<20} {ref_vel*100:>9.2f}  {vel_best*100:>9.2f}  {vel_worst*100:>9.2f}  {vel_spread*100:>7.2f}  {ref_err_pct:>+7.1f}%")
...
if results:
    mean_err = np.mean([abs(r["ref_err_pct"]) for r in results])
    max_err = np.max([abs(r["ref_err_pct"]) for r in results])
    mean_spread = np.mean([r["spread"] * 100 for r in results])

    print(f"\nSUMMARY:")
    print(f"  Mean absolute ref error: {mean_err:.1f}%")
