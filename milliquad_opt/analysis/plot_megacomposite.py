"""Megacomposite: 3 terrains × 5 columns (vel exp|sim, pitch exp|sim, COT sim).

Layout (3×5):
              vel_exp    vel_sim    pitch_exp   pitch_sim   COT
    flat        ✓          ✓          ✓           ✓          ✓
    step        ✓          ✓          ✓           ✓          ✓
    rough       ✓          ✓        blank         ✓          ✓
...
Usage:
    uv run python -m analysis.plot_megacomposite \
...
def main():
    ...
    n_rows = len(present)
    n_cols = 5  # vel_exp, vel_sim, pitch_exp, pitch_sim, cot
    ...
    for i, terrain in enumerate(present):
        ...
        plot_panel(axes[i, 4], cot_data, f"{title}: COT", "Cost of Transport", all_failed=all_failed, scatter_only=so)
    ...
    fig.tight_layout()
    out = args.output or "plots/megacomposite.png"
