"""Step terrain optimization config.
...
# ---------------------------------------------------------------------------
# Cost function (spatial-gated, step-aware)
# ---------------------------------------------------------------------------
...
def calculate_cost(
    ...
    # Exit state: last state within 90% of step region (avoids cliff-edge artifacts)
    ...
    # Tumble penalty (within 90% of step region)
    tumble_penalty = 0.0
    tumble_count = 0
    for state in trajectory:
        if state["pos"][0] > active_cutoff:
